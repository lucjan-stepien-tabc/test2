###
# This is the place to override the apache2 cookbook's default attributes.
#
# Do not edit THIS file directly. Instead, create
# "apache2/attributes/customize.rb" in your cookbook repository and
# put the overrides in YOUR customize.rb file.
###

# The following shows how to override the Apache contact and timeout settings:
#
#normal[:apache][:contact] = 'you@your-site.com'
#normal[:apache][:timeout] = 60

# Prefork Attributes
# normal[:apache][:prefork][:startservers]
# normal[:apache][:prefork][:minspareservers]
# normal[:apache][:prefork][:maxspareservers]
# normal[:apache][:prefork][:serverlimit]
# normal[:apache][:prefork][:maxclients]
# normal[:apache][:prefork][:maxrequestworkers]
# normal[:apache][:prefork][:maxrequestsperchild]
# normal[:apache][:prefork][:maxconnectionsperchild]
normal[:apache][:contact] = 'lucjan.stepien@test.com'
# normal[:apache][:user]    = 'tix-wsgi'
# normal[:apache][:group]   = 'tix-wsgi'